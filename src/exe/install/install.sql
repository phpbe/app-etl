...
`sample_data` text NOT NULL COMMENT '示例数据',
`output_data` text NOT NULL COMMENT '输出数扰',
`create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
`update_time` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
PRIMARY KEY (`id`)
...
`field_mapping` varchar(30) NOT NULL DEFAULT 'same' COMMENT '字段映射类型（mapping：字段映射/code：代码处理）',
...
`flow_id` varchar(36) NOT NULL DEFAULT '' COMMENT '数据流ID',
`flow_node_id` varchar(36) NOT NULL DEFAULT '' COMMENT '数据流节点ID',
`field` varchar(30) NOT NULL DEFAULT '' COMMENT '取用字段',
`file_ext` varchar(30) NOT NULL DEFAULT '' COMMENT '扩展名',
`sample_data` text NOT NULL COMMENT '示例数据',
`output_data` text NOT NULL COMMENT '输出数扰',
`create_time` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
`update_time` TIMESTAMP on update CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
