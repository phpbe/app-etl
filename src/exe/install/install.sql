...
CREATE TABLE `etl_extract` (
  ...
  `src_type` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '来源数据源类型（0 - 表 / 1：SQL',
  ...
  `field_mapping_type` tinyint(3) unsigned NOT NULL DEFAULT '0' COMMENT '字段映射类型（0:不需要映射/1：字段映射/2：代码处理）',
  ...
  `breakpoint` DATETIME NOT NULL DEFAULT '1970-01-02 00:00:00' COMMENT '断点',
...
CREATE TABLE `etl_extract_exception` (
  ...
  `extract_id` int(11) NOT NULL DEFAULT '0' COMMENT '抽取任务ID',
  `message` varchar(200) NOT NULL DEFAULT '' COMMENT '异常信息',
  `trace` text NOT NULL COMMENT '异常跟踪信息',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_general_ci COMMENT='抽取任务执行异常记录';
